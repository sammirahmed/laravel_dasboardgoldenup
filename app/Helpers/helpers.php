...
{
    if (isset($column)) {
        if ($column[$attributeName]=='active') {
            return "<span class='badge badge-success'>" . trans('نشط') . "</span>";
        } else {
            return "<span class='badge badge-danger'>" . trans('منتهي') . "</span>";
        }
    }
}
...
{
    $html = '<a href="${href}" class="text-bold text-primary">${title}</a>';
    $result = [];
    foreach ($array as $link) {
...
function getLinksColumnByRouteNameonly($link, $routeName, $idAttribute = 'id', $titleAttribute = 'title')
{
    $html = '<a href="${href}" class="text-bold text-primary">${title}</a>';
    $result = [];
   {
...
function getCustomFieldsValuesex($customFields = null, $request = null)
{
