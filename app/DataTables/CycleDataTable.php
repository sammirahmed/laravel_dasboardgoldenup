...
class CycleDataTable extends DataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $dataTable = $dataTable
            ...
            ->addColumn('action', 'cycles.datatables_actions')
    ...
    protected function getColumns()
    {
        ...
        $hasCustomField = in_array(Cycle::class, setting('custom_field_models',[]));
        if ($hasCustomField) {
            $customFieldsCollection = CustomField::where('custom_field_model', Cycle::class)->where('in_table', '=', true)->get();
            foreach ($customFieldsCollection as $key => $field) {
                array_splice($columns, $field->order - 1, 0, [[
                    'data' => 'custom_fields.' . $field->name . '.view',
                    'title' => trans('lang.cycle_' . $field->name),
                    'orderable' => false,
                    'searchable' => false,
                ]]);
            }
