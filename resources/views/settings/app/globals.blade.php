        ...
        <div class="card-body">
            ...
            <div class="row">
                <div style="flex: 50%;max-width: 50%;padding: 0 4px;" class="column">
                    ...
                    <!-- app_action1 name -->
                    <div class="form-group row">
                        {!! Form::label('app_name', trans("Share Button Text"), ['class' => 'col-4 control-label text-right']) !!}
                        <div class="col-8">
                            {!! Form::text('action_b_name', setting('action_b_name'),  ['class' => 'form-control','placeholder'=>  trans("Enter Share text")]) !!}
                            <div class="form-text text-muted">
                                {{ trans("Share Button Text") }}
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- app_action1 sub -->
                    <div class="form-group row">
                        {!! Form::label('app_short_description', trans("Share Subject"), ['class' => 'col-4 control-label text-right']) !!}
                        <div class="col-8">
                            {!! Form::textarea('action_b_sub', setting('action_b_sub'),  ['class' => 'form-control','placeholder'=>  trans("Enter Share Subject")]) !!}
                            <div class="form-text text-muted">
                                {{ trans("Share Subject") }}
                            </div>
                        </div>
                ...
                <div style="flex: 50%;max-width: 50%;padding: 0 4px;" class="column">
                    ...
                    <div class="form-group row">
                        {!! Form::label('app_logo', trans("lang.upload_app_logo1"), ['class' => 'col-4 control-label text-right']) !!}
                        <div class="col-8">
                            <div style="width: 100%" class="dropzone app_action1" id="app_action1" data-field="app_action1">
                                <input type="hidden" name="app_action1">
                            </div>
                            <a href="#loadMediaModal" data-dropzone="app_action1" data-toggle="modal" data-target="#mediaModal" class="btn btn-outline-{{setting('theme_color')}} btn-sm float-right mt-1">{{ trans('lang.media_select')}}</a>
                            <div class="form-text text-muted w-50">
                                {{ trans("lang.upload_app_logo_help") }}
                            </div>
                        </div>
                    ...
                    <div class="form-group row">
                        {!! Form::label('app_logo', trans("lang.upload_app_logo2"), ['class' => 'col-4 control-label text-right']) !!}
                        <div class="col-8">
                            <div style="width: 100%" class="dropzone app_action2" id="app_action2" data-field="app_action2">
                                <input type="hidden" name="app_action2">
                            </div>
                            <a href="#loadMediaModal" data-dropzone="app_action2" data-toggle="modal" data-target="#mediaModal" class="btn btn-outline-{{setting('theme_color')}} btn-sm float-right mt-1">{{ trans('lang.media_select')}}</a>
                            <div class="form-text text-muted w-50">
                                {{ trans("lang.upload_app_logo_help") }}
                            </div>
                        </div>
                    ...
                    <script type="text/javascript">
                        ...
                        var dzAppLogoaction = '';
                        @if(isset($uploadaction) && $uploadaction->hasMedia('app_action1'))
                            dzAppLogoaction = {
                            name: "{!! $uploadaction->getFirstMedia('app_action1')->name !!}",
                            size: "{!! $uploadaction->getFirstMedia('app_action1')->size !!}",
                            type: "{!! $uploadaction->getFirstMedia('app_action1')->mime_type !!}",
                            collection_name: "{!! $uploadaction->getFirstMedia('app_action1')->collection_name !!}"
                        };
                                @endif
                        var dz_dzAppLogoaction  = $(".dropzone.app_action1").dropzone({
                                url: "{!!url('uploads/store')!!}",
                                addRemoveLinks: true,
                                maxFiles: 1,
                                init: function () {
                                    @if(isset($uploadaction) && $uploadaction->hasMedia('app_action1'))
                                    dzInit(this, dzAppLogoaction, '{!! url($uploadaction->getFirstMediaUrl('app_action1','thumb')) !!}')
                                    @endif
                                },
                                accept: function (file, done) {
                                    dzAccept(file, done, this.element, "{!!config('medialibrary.icons_folder')!!}");
                                ...
                                maxfilesexceeded: function (file) {
                                    dz_dzAppLogoaction[0].mockFile = '';
                                    dzMaxfile(this, file);
                                },
                                complete: function (file) {
                                    dzComplete(this, file, dzAppLogoaction, dz_dzAppLogoaction[0].mockFile);
                                    dz_dzAppLogoaction[0].mockFile = file;
                                },
                                removedfile: function (file) {
                                    dzRemoveFile(
                                        file, dzAppLogoaction, '{!! url("uploads/remove-media") !!}',
                                        'app_action1', '{!! isset($uploadaction) ? $uploadaction->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                                    );
                                }
                            });
                        dz_dzAppLogoaction[0].mockFile = dzAppLogoaction;
                        dropzoneFields['app_action1'] = dz_dzAppLogoaction;



                        var dzAppLogoaction2 = '';
                        @if(isset($uploadaction2) && $uploadaction2->hasMedia('app_action2'))
                            dzAppLogoaction2 = {
                            name: "{!! $uploadaction2->getFirstMedia('app_action2')->name !!}",
                            size: "{!! $uploadaction2->getFirstMedia('app_action2')->size !!}",
                            type: "{!! $uploadaction2->getFirstMedia('app_action2')->mime_type !!}",
                            collection_name: "{!! $uploadaction2->getFirstMedia('app_action2')->collection_name !!}"
                        };
                                @endif
                        var dz_dzAppLogoaction2  = $(".dropzone.app_action2").dropzone({
                                url: "{!!url('uploads/store')!!}",
                                addRemoveLinks: true,
                                maxFiles: 1,
                                init: function () {
                                    @if(isset($uploadaction2) && $uploadaction2->hasMedia('app_action2'))
                                    dzInit(this, dzAppLogoaction2, '{!! url($uploadaction2->getFirstMediaUrl('app_action2','thumb')) !!}')
                                    @endif
                                },
                                accept: function (file, done) {
                                    dzAccept(file, done, this.element, "{!!config('medialibrary.icons_folder')!!}");
                                ...
                                maxfilesexceeded: function (file) {
                                    dz_dzAppLogoaction2[0].mockFile = '';
                                    dzMaxfile(this, file);
                                },
                                complete: function (file) {
                                    dzComplete(this, file, dzAppLogoaction2, dz_dzAppLogoaction2[0].mockFile);
                                    dz_dzAppLogoaction2[0].mockFile = file;
                                },
                                removedfile: function (file) {
                                    dzRemoveFile(
                                        file, dzAppLogoaction2, '{!! url("uploads/remove-media") !!}',
                                        'app_action2', '{!! isset($uploadaction2) ? $uploadaction2->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                                    );
                                }
                            });
                        dz_dzAppLogoaction2[0].mockFile = dzAppLogoaction2;
                        dropzoneFields['app_action2'] = dz_dzAppLogoaction2;
