...
function dzRemoveFiles(file, mockFile = '', existRemoveUrl = '', collection, modelId,_uuid, newRemoveUrl, csrf) {
    if (file.previewElement != null && file.previewElement.parentNode != null) {
        file.previewElement.parentNode.removeChild(file.previewElement);
    }
    //if(file.status === 'success'){
...
function dzSendingmultiaple(_this, files, formData, csrf) {
    _this.element.children[1].value =files[0].upload.uuid;
    formData.append('_token', csrf);
    formData.append('field', _this.element.dataset.field);
    formData.append('uuid', files[0].upload.uuid);
}
...
function dzInitz(_this,mockFile,thumb) {
    _this.options.addedfile.call(_this, mockFile);
    _this.options.thumbnail.call(_this, mockFile, thumb);
    mockFile.previewElement.classList.add('dz-success');
    mockFile.previewElement.classList.add('dz-complete');
